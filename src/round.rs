	...
	Signature: Eq + Clone,
{
	...
	/// Return the primary index of the round.
	pub fn primary_voter(&self) -> &(Id, u64) {
		self.voters.voters
			.get(self.round_number as usize % self.voters.voters.len())
			.expect("by definition of % and length of self.voters.voters; qed")
	}
