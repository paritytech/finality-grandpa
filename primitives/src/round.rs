...
impl VoteWeight {
	...
	fn total_weight<Id: Hash + Eq + Ord>(
		&self,
...
impl<Id: Hash + Eq + Clone + Ord, Vote: Clone + Eq, Signature: Clone + Eq> VoteTracker<Id, Vote, Signature> {
	fn new() -> Self {
		VoteTracker {
			votes: BTreeMap::new(),
			current_weight: 0,
	...
	fn update(&mut self) {
		...
		let possible_to_precommit = {
			...
			move |weight: &VoteWeight| {
				...
				let possible_equivocations = core::cmp::min(
